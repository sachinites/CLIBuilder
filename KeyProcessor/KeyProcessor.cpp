...
cli_application_process (cli_t *cli) {
    ...
    if (cli_cursor_is_at_begin_of_line (cli)) return -1;
        ...
        ch = getch();
         ...
         switch (ch) {
            ...
            case KEY_HOME: /* Inbuilt , provided by ncurses */
                if (cli_cursor_is_at_begin_of_line(NULL)) break;
            ...
            if (default_cli_history_list->curr_ptr == NULL) {
                ...
                cli_screen_cursor_save_screen_pos(default_cli);
                move(default_cli->row_store + 1, 0);
                cli_content_reset (NULL);
                cli_printsc (NULL, false);
            }
            else {
                 assert(cli_store);
